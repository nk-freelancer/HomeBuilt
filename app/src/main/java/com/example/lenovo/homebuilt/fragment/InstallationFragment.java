...
public class InstallationFragment extends Fragment implements IncDescQuantity, View.OnClickListener {

    TextView cart_price, view_cart;
    ListView installation_service_list;
    ...
    private void initVar(View view) {
        installation_service_list = (ListView) view.findViewById(R.id.installation_service_list);
        ...
        try {
            ...
            for (int i = 0; i < serviceArray.length(); i++) {
                ...
                bsInstallationArrayList.add(new BSInstallation(service_item, 0,
                        Double.parseDouble(installation_charge)));
            }
        } catch (JSONException e) {
            e.printStackTrace();
        }
        if (bsInstallationArrayList.isEmpty())
            installation_service_list.setEmptyView(view.findViewById(R.id.emptyElement));
        else
            installation_service_list.setAdapter(new BSInstallationAdapter(getContext(), bsInstallationArrayList, this));
    ...
    public void onClick(View v) {
        if (sharePref.getStatus()) {
            ...
            intent.putExtra("service_type", "installation");
